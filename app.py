...
if pdf_file is not None:
    try:
        ...
        if pos:
            ...
            # --- STEP 2: PATHS ---
            st.divider()
            st.subheader("2. Paste paths then press Ctrl + Enter")
            ...
            for po in pos:
                ...
                for p_str in clean_p:
                    if po in p_str:
                        match = p_str
                        break
                
                info = extract_path_info(match)
                results.append({"PO Number": po, "TO_COPY": info})
            ...
            st.download_button("📥 Download Results CSV", data=csv, file_name="trixie_results.csv")
        else:
